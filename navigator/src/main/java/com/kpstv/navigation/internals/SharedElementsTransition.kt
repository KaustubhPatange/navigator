...
internal fun FragmentTransaction.prepareForSharedTransition(fm: FragmentManager, containerView: FrameLayout, clazz: FragClazz, payload: AnimationDefinition.Shared) {
    ...
    currentFragment?.let {
        if (payload.exit != -1) currentFragment.exitTransition =containerView.context.inflateTransition(payload.exit)
    }
...
internal class SharedElementCallback(
    ...
    override fun onFragmentCreated(fm: FragmentManager, f: Fragment, savedInstanceState: Bundle?) {
        ...
        if (f::class == clazz) {
            if (payload.enter != -1) f.enterTransition = f.requireContext().inflateTransition(payload.enter)
